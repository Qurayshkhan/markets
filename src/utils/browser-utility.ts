...
export const setOrgMetadata = (obj: any) => {
  window.localStorage.setItem(`${storageKey}-metadata`, JSON.stringify(obj))
}

export const getOrgMetadata = (): any => {
  if (typeof window !== "undefined") {
    const temp = window.localStorage.getItem(`${storageKey}-metadata`)
    if (temp) return JSON.parse(temp)
    else return null
  }
}
